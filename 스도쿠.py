#https://www.acmicpc.net/problem/2580
'''
어케함 ㅇ?
가로순회, 박스 순회해서
1개 남은거 찾고 채우기

백트래킹으로 하려면 매번 저 2차원 배열을 채우고 다시넣고 한다?
박스를 어떻게 나눠서 체크하지?
만약 1,2 가 없는데 이걸 어떻게 알지?
'''
def box_check(box_pan):
    for x in range(3):
        for y in range(3):
            S = list()
            for in_x in range(3):
                for in_y in range(3):
                    S.append(box_pan[3*x + in_x][3*y + in_y])
            if (0 in S) and len(set(S)) != 9:
                return False
    else:
        return True

def col_check(col_pan):
    for x in range(9):
        S = list()
        for y in range(9):
            S.append(col_pan[y][x])
        if 0 in S and len(set(S)) != 9:
            return False
    else:
        return True

def sdoku(sdoku_pan):
    tmp_sdoku = sdoku_pan
    if box_check(sdoku_pan) and col_check(sdoku_pan):
        return sdoku_pan

    for num_li in range(9):
        if 0 in sdoku_pan[num_li]:
            tmp_set = set(sdoku_pan[num_li])
            need_num =list(standard - tmp_set)
            print(need_num) # [2,9]
            Flag = False
            for position in range(9):
                if sdoku_pan[num_li][position] == 0:
                    for n in need_num:
                        sdoku_pan[num_li][position] = n
                        print(n)
                        sdoku(sdoku_pan)


li = list()
for i in range(9):
    li.append(list(map(int, input().split())))

standard ={1,2,3,4,5,6,7,8,9}
sdoku(li)

print(li)


##################

sudoku = [list(map(int, input().split())) for _ in range(9)]
#해결해야될 칸만 받음
zeros = [(i, j) for i in range(9) for j in range(9) if sudoku[i][j] == 0]

def is_promising(i, j):
    promising = [1,2,3,4,5,6,7,8,9]  
    
    #행열 검사
    for k in range(9):
        if sudoku[i][k] in promising:
            promising.remove(sudoku[i][k])
        if sudoku[k][j] in promising:
            promising.remove(sudoku[k][j])
            
    #3*3 박스 검사
    i //= 3
    j //= 3
    for p in range(i*3, (i+1)*3):
        for q in range(j*3, (j+1)*3):
            if sudoku[p][q] in promising:
                promising.remove(sudoku[p][q])
    
    return promising

flag = False #답이 출력되었는가?
def dfs(x):
    global flag
    
    if flag: #이미 답이 출력된 경우
        return
        
    if x == len(zeros): #마지막 0까지 다 채웠을 경우
        for row in sudoku:
            print(*row)
        flag = True #답 출력
        return
        
    else:    
        (i, j) = zeros[x]
        promising = is_promising(i, j) #유망한 숫자들을 받음
        
        for num in promising:
            sudoku[i][j] = num #유망한 숫자 중 하나를 넣어줌
            dfs(x + 1) #다음 0으로 넘어감
            sudoku[i][j] = 0 #초기화 (정답이 없을 경우를 대비)
dfs(0)