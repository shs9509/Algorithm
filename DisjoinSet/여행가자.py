...
N = int(input()) # 노드 수 입력 받기// 3
M = int(input()) # 정점 수 입력 받기// 3
parent = [0] * (N+1) # 부모 테이블 초기화// [0,0,0,0]
...
def union(a, b): # a집합과 b집합 합치기
    ...
    if a < b: # a의 부모가 b 부모보다 상위 루트이면
        parent[b] = a # b의 부모를 a의 부모로 변경// parent[2] =1 =>  [1,1,1,3]  
    else: # b의 부모가 a 부모보다 상위 루트이면
        parent[a] = b # a의 부모 변경

for y in range(1, N+1): # y번째 도시
    maps = list(map(int, input().split())) # y번째 도시가 어느 도시와 연결되어 있는지 정보 //[0,1,0]
    for x in range(1, len(maps)+1): # y도시 연결 정보 추출//(1,4)
        if maps[x-1] == 1: # x 도시와 연결되어 있으면 // maps[0] [1] [2]
            union(y, x) # 두 도시를 결합 union(1,2)

tour = list(map(int, input().split())) # 여행 계획 정보 //[1,2,3]
